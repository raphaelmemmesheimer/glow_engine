...
#define positionTexture sPosition
#define normalTexture sNormal
#define colorTexture sScene
#define gl_FragCoord oTextureCoord
#define envmapTexture tex00
#define seedTexture tex01
#define modelviewMatrix Modelview
#define projectionMatrix Projection
#define directRadianceTexture sRadiance
...
uniform float singularity = 50.0;

uniform float depthBias = 1.0;

// Strength of the indirect bounce
uniform float bounceStrength = 10700.0f;

uniform float bounceSingularity = 2700.0f;
...
void main() {
	...
	vec3 normal = sampleNormal(oTextureCoord.xy); 
	vec4 position = samplePosition(oTextureCoord.xy);
	vec4 pixelColor = sampleColor(oTextureCoord.xy);           
	...
	if(pixelColor.a > 0.0) {
		...
		int patternIndex = int(oTextureCoord.x) % patternSize + (int(gl_FragCoord.y) % patternSize) * patternSize;
		...
		for(int i = 0; i < sampleCount ; i++) {
			...
			vec2 occlusionTexCoord = textureSize2D(sPosition, 0) * (vec2(0.5) + 0.5 * (occluderSamplePosition.xy / occluderSamplePosition.w));
...
asd
   	//FragColor = texture(seedTexture,gl_FragCoord);
}