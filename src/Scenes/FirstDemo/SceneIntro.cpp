...
void SceneIntro::init() {
	...
	SceneObjectNode* obj = new SceneObjectNode();
	ObjModel* m  = new ObjModel();
	m->loadFile("/home/raphael/models/obj/airglow.obj");
	obj->setDrawable(m);

    printf("%s\n", "init SceneGraph");
	SceneTranslateNode* root = new SceneTranslateNode();
	root->setTranslateValues(10.0, 0.0, 0.0);

	//sceneGraph.init((SceneNode*)obj);

	sceneGraph.init((SceneNode*)root);
	((SceneNode*)(root))->addNode((SceneNode*)obj);

	//printf("%s\n", sceneGraph.getRootNode()->getName().c_str()); 
	printf("boom\n");
	...
	quad = new Quad(0.2);
	model = new ObjModel();
	model->loadFile("data/FirstDemo/abstract.obj");
...
void SceneIntro::resize(int width, int height){
	glViewport(0, 0, width, height);
    engine->setHeight(height);
    engine->setWidth(width);
}

void SceneIntro::handleMousePos(int x, int y){

}
void SceneIntro::handleMouseScroll(int hor, int vert){

}
void SceneIntro::handleMouseWheel(int mouseWheelPos){

}

void SceneIntro::handleKeyboard(int key, int action) {
	printf("key %d \n" ,key);
	zoom+=0.001;
}
