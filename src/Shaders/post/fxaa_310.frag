...
{   
    ...
    FragColor = FragColor.a*FragColor+((1-FragColor.a)*vec4(oTextureCoord.y,oTextureCoord.y,oTextureCoord.y,0.0));
    FragColor.rgba = texture(sScene, oTextureCoord);
    FragColor.a = dot(FragColor.rgb, float3(0.299, 0.587, 0.114)); // compute luma
    
    FragColor.rgba = FxaaPixelShader(oTextureCoord,sScene,1/resolution);//brightness*pow(tmpColor,tmpSaturation);
}  