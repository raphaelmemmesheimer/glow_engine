...
void main(void)
{
    ...
    vec2 s = p;
    p = p.yx*tan(p.xy*time/5);
    float color = superformula(1.0,1.0,8,theta(p.x,p.y),sin(time),sin(time),8)*calcr(p.x,p.y);
    gl_FragColor = vec4(color,color*color,color*color,1.0);
}
